...
if [ -n "$LAST_SYNCED" ]; then
  git log --oneline $LAST_SYNCED..HEAD -- empresa/contexto/ agentes/aria/memory/
else
  git log --oneline HEAD -- empresa/contexto/ agentes/aria/memory/
fi
```
...
> _[1] Time (cérebro da empresa) — inbox/fabianafurtadoff/_
> _[2] Diretoria — inbox/fabianafurtadoff/_
> _[3] Ambos_
...
1. Copiar o arquivo (Markdown/HTML) pra `inbox/fabianafurtadoff/` do repo-destino
2. Gerar sidecar `.meta.yaml` com:

```yaml
autor: fabianafurtadoff
...
git add inbox/fabianafurtadoff/
git commit -m "team-sync: Fabiana Furtado ({N} capturas)"
...
> _- {N} capturas enviadas pro time (inbox/fabianafurtadoff/ em staging)_
> _- {M} capturas enviadas pra diretoria (inbox/fabianafurtadoff/ em staging)_
...
- **Conflito em `inbox/fabianafurtadoff/` (outro sócio com mesmo slug?)** — não deveria acontecer; abortar com alerta
