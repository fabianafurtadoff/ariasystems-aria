...
Membros do time que só têm 1 cérebro (`ariasystems-second-brain-time`) também podem rodar — nesse caso é no-op (Claude Code já carrega o CLAUDE.md do cwd se a sessão abriu direto no repo). Útil mesmo pra 3 cérebros.
...
4. Confirmar em **1 linha curta**, listando agente de cada cérebro:
   ...
   > 3 cérebros carregados: Aria (pessoal), Sansa (time), Brian (diretoria).

   Se faltou algum:

   > 2/3 cérebros carregados: Aria ✓, Sansa ✓, diretoria ✗ (symlink quebrado).
...
- Não cria daily note — Aria faz sob demanda.
