...
*/15 * * * * cd ~/ffos/ffos-projects/brains/{{EMPRESA_SLUG}}-{{AGENTE_SLUG}} && /path/to/autosave-and-sync.sh

# 1x/dia às 09:00: team-sync com quiz (se houver coisa nova)
0 12 * * * cd ~/ffos/ffos-projects/brains/{{EMPRESA_SLUG}}-{{AGENTE_SLUG}} && /path/to/team-sync.sh --interactive
```
